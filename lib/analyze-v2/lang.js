/**
 * UI may send "TR", "tr", "turkish"; normalize for engines.
 * @param {unknown} lang
 * @returns {"en" | "tr"}
 */
export function normalizeAnalyzeLang(lang) {
  const s = String(lang ?? "")
    .trim()
    .toLowerCase();
  if (s === "tr" || s === "tur" || s === "turkish") return "tr";
  return "en";
}

/** UI Turkish: prepend to prompts so every model output is Turkish (incl. skill labels). */
export const MANDATORY_TURKISH_AI_OUTPUT = `Sen Türkçe konuşuyorsun.
Tüm çıktılar Türkçe olmalı.
Hiçbir cümle İngilizce olamaz.
Skill isimleri dahil her şey Türkçe.`;

/**
 * Mandatory top-line system directive requested by product UX.
 * @param {"en" | "tr"} langNorm
 */
export function requiredResponseLanguageDirective(langNorm) {
  const language = langNorm === "tr" ? "Turkish" : "English";
  return `You must respond entirely in ${language}.
Every single word of your response must be in this language.
This is mandatory. Do not use any other language under any circumstances.`;
}

/**
 * Single output language (UI language). Use in every Claude system + user prompt.
 * @param {"en" | "tr"} langNorm
 */
export function criticalOutputLanguageInstruction(langNorm) {
  if (langNorm === "tr") {
    return `CRITICAL: Respond ONLY in Turkish.
If language is Turkish: Her cümle Türkçe olmalı.
No mixing. No switching mid-sentence.`;
  }
  return `CRITICAL: Respond ONLY in English.
No mixing. No switching mid-sentence.`;
}

/** Appended to system prompts when lang is Turkish. */
export const TR_JSON_OUTPUT_RULE =
  "Respond in Turkish (tr). All output must be in Turkish. Every natural-language string value in your JSON (reasoning, issues, explanations, matched_skills, missing_keywords, top_keywords, parsing_issues, strengths, weaknesses, red_flags, biggest_gap, action_plan.priority_callout, action_plan.fixes[].issue, action_plan.fixes[].steps[], action_plan.fixes[].priority, action_plan.fixes[].score_impact, action_plan.fixes[].resource.label, action_plan.interview_note, role labels if natural language, etc.) must be written in Turkish — use Turkish skill phrases (e.g. veri analizi, proje yönetimi); only universal tokens like SQL, AWS, API may stay as symbols. Keep JSON keys exactly as specified in English.";
export const EN_JSON_OUTPUT_RULE =
  "Respond in English (en). All output must be in English. Keep JSON keys exactly as specified in English.";

/**
 * @param {string} baseSystem
 * @param {"en" | "tr"} langNorm
 */
export function systemPromptWithLang(baseSystem, langNorm) {
  const trLead = langNorm === "tr" ? `${MANDATORY_TURKISH_AI_OUTPUT}\n\n` : "";
  const requiredDirective = requiredResponseLanguageDirective(langNorm);
  const critical = criticalOutputLanguageInstruction(langNorm);
  const mustRule =
    langNorm === "tr"
      ? "You MUST respond entirely in Turkish language. Every single word must be in Turkish."
      : "You MUST respond entirely in English language. Every single word must be in English.";
  const languageRule = langNorm === "tr" ? TR_JSON_OUTPUT_RULE : EN_JSON_OUTPUT_RULE;
  return `${requiredDirective}\n\n${trLead}${critical}\n\n${mustRule}\n${baseSystem}\n\n${languageRule}`;
}

export function fallbackNoReasoning(langNorm) {
  return langNorm === "tr"
    ? "Recruiter profilini tamamen uzak görmeyebilir. Ama role özel sinyaller yeterince net olmadığı için karar aşamasında tereddüt yaşayabilir."
    : "A recruiter may not see your profile as fully misaligned, but uncertainty can remain when role-specific signals are not explicit enough.";
}

export function fallbackBiggestGap(langNorm) {
  return langNorm === "tr"
    ? "Tek bir boşluk izole edilemedi."
    : "Unable to isolate a single gap.";
}

/** Appended to user prompts as a second reminder for models that skim system text. */
export function userPromptLangFooter(langNorm) {
  const responseLanguage = langNorm === "tr" ? "Turkish" : "English";
  return `IMPORTANT: Respond only in ${responseLanguage}.`;
}

export function enforcePromptLanguageRules(messages, langNorm = "en") {
  const normalizedLang = normalizeAnalyzeLang(langNorm);
  const systemLead = requiredResponseLanguageDirective(normalizedLang);
  const userTail = userPromptLangFooter(normalizedLang);
  const cloned = Array.isArray(messages)
    ? messages.map((m) => ({ ...m, content: String(m?.content ?? "") }))
    : [];

  const first = cloned[0];
  if (!first || String(first.role || "").toLowerCase() !== "system") {
    cloned.unshift({ role: "system", content: systemLead });
  } else {
    const current = String(first.content || "");
    if (!current.startsWith(systemLead)) {
      first.content = `${systemLead}\n\n${current}`.trim();
    }
  }

  return cloned.map((m) => {
    if (String(m.role || "").toLowerCase() !== "user") return m;
    const content = String(m.content || "").trim();
    if (content.endsWith(userTail)) return { ...m, content };
    return { ...m, content: `${content}\n\n${userTail}`.trim() };
  });
}
