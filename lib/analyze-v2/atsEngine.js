...
export async function runAtsEngine(cvText, jobDescription, sector, langNorm = "en", careerContext = null) {
  ...
  const genericBaselineMissingRule =
    langNorm === "tr"
      ? `EKSİK SKILL SATIRLARI (missing_keywords*, missing_keywords_hard, missing_keywords_soft): Yalnızca şunları yaz — (1) teknik araçlar ve sorgu/diller (ör. SQL, Python), (2) adlandırılmış mesleki sertifikalar, (3) JD ile doğrudan bağlantılı alan metodolojileri, (4) yazılım platformları. İlan dolgusu veya genel davranış/kişilik ifadeleri bu dizilere konmaz.`
      : `MISSING-KEYWORD ROWS (missing_keywords*, missing_keywords_hard, missing_keywords_soft): Include ONLY — (1) technical tools and languages (e.g. SQL, Python), (2) named professional certifications, (3) domain-specific methodologies clearly required by the JD, (4) software platforms. Do not put JD boilerplate or generic soft-skill rows in these arrays.`;
...
Return ONLY valid JSON:
{
  ...
  const system = systemPromptWithLang(
    "You are an ATS parser simulator. Focus ONLY on skills and keyword matching evidence. Recruiters rely on keyword filters. Call out parsing and keyword failures bluntly — this CV either clears the bot or it does not. For missing_keywords and top_keywords, output ONLY actionable skills (tools, languages, methods, certs). NEVER list university departments, degree names, major titles, class year, or eligibility lines as missing skills. Exclude education requirements, benefits, policies, internships-as-condition, insurance, or non-skill JD boilerplate. missing_* arrays must contain ONLY technical tools/languages, named certifications, JD-specific domain methodologies, or software platforms — not generic soft skills or JD boilerplate. parsing_issues must use the same UI output language as all other natural-language list fields (English vs Turkish per CRITICAL instruction). Return valid JSON only. Be specific to THIS candidate's CV.",
    langNorm
  );
  const content = await callClaude(user, system, 1400, { langNorm, model: MODEL });
