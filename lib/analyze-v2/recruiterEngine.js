...
export async function runRecruiterEngine(cvText, jobDescription, sector) {
...
Return ONLY valid JSON:
{
      ...
      messages: [
        {
          role: "system",
          content:
            'Verdict must be exactly "strong_yes", "maybe", or "no". No fluff. JSON only.',
        },
      ...
      messages: [
        {
          role: "system",
          content:
            'Verdict must be exactly "strong_yes", "maybe", or "no". JSON only.',
        },
        { role: "user", content: user },
      ],
    });
  ...
  return {
    ...
    reasoning: String(p.reasoning || "").trim() || "No reasoning returned.",
