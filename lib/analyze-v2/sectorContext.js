import { buildCareerMemoryPromptBlock } from "../careerMemory/index.js";
import { buildOnboardingPromptBlock } from "../careerOnboarding/personalizedInsights.js";
...
export function getSectorPromptBlock(sector, careerContext = null) {
  ...
  const memoryBlock = buildCareerMemoryPromptBlock(careerContext?.careerMemory);
  const onboardingBlock = buildOnboardingPromptBlock(careerContext?.careerMemory);
  return `${base}${buildCareerAreaContextBlock(careerContext)}${memoryBlock}${onboardingBlock}`;
}
