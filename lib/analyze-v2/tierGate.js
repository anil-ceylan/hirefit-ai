...
export function buildUnifiedResponse(ats, recruiter, gaps, roleFit, decision, sector, careerContext = null) {
  ...
  return {
    ...
    Recruiter: {
      ...
      structured_analysis: recruiter.structured_analysis || null,
    },
    Gaps: {
...
export function applyTierGate(full, isPro, lang = "en") {
  ...
  return {
    ...
    Recruiter: {
      ...
      red_flags: [],
      structured_analysis: {
        first_perception: String(full.Recruiter?.structured_analysis?.first_perception || "").trim(),
        internal_monologue: ensureNarrative(
          full.Recruiter?.structured_analysis?.internal_monologue || full.Recruiter?.reasoning,
          langNorm,
          300
        ),
        core_concern: String(full.Recruiter?.structured_analysis?.core_concern || "").trim(),
        persuasion_tip: String(full.Recruiter?.structured_analysis?.persuasion_tip || "").trim(),
        signal_tags: Array.isArray(full.Recruiter?.structured_analysis?.signal_tags)
          ? full.Recruiter.structured_analysis.signal_tags.slice(0, 4)
          : [],
      },
    },
    Gaps: {
