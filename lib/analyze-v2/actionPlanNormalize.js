...
/**
 * @param {unknown} raw
 * @returns {{ priority_callout: string | null, fixes: Array<{ issue: string, severity: string, priority: string, score_impact: number, steps: string[], resource: { label: string, url: string | null } | null }>, interview_note: string | null }}
 */
function normScoreImpact(raw, severity) {
  const n = Number(raw);
  if (Number.isFinite(n) && n > 0) return Math.max(1, Math.min(18, Math.round(n)));
  ...
  try {
    ...
    const normFixItem = (f) => {
      ...
      return {
        ...
        score_impact: normScoreImpact(f?.score_impact, severity),
        steps,
...
/** Ensure high-priority fix has the strongest score_impact (model ordering guard). */
function rebalanceScoreImpacts(fixes) {
  const out = fixes.map((f) => ({
    ...f,
    score_impact: normScoreImpact(f?.score_impact, f.severity || "major"),
  }));
  const hi = out.findIndex((f) => f.priority === "high");
  if (hi === -1) return out;
  const maxOthers = Math.max(
    0,
    ...out.map((f, i) => (i === hi ? 0 : Number(f.score_impact) || 0))
  );
  const cur = Number(out[hi].score_impact) || 0;
  if (cur <= maxOthers) {
    out[hi] = { ...out[hi], score_impact: Math.min(18, maxOthers + 1) };
  }
  return out;
}
...
export function enrichActionPlan(plan, ctx = {}) {
  ...
  fixes = rebalanceScoreImpacts(fixes);
  return {
