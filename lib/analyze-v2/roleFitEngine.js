...
export async function runRoleFitEngine(cvText, jobDescription, sector) {
...
{
  "role_fit": [
    { "role": "Product", "score": <number> },
    { "role": "Marketing", "score": <number> },
    { "role": "Data", "score": <number> },
    { "role": "Strategy", "score": <number> }
  ],
  "best_role": "<one of the four OR a tighter label if clearly different, e.g. Engineering>"
}

If CV is clearly engineering, you may set low scores across all four and set best_role to the real lane (still one short label).`;
    ...
    messages: [
      {
        role: "system",
        content:
          "You map CV evidence to role families. No encouragement. JSON only.",
      },
