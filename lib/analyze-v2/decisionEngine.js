...
function buildDecisionReasoningFallback({ recruiter, recruiterView, gapSignals, langNorm }) {
  ...
  if (rv) {
    return langNorm === "tr"
      ? `${rv} Bu yüzden karar anında risk algısı yükseliyor ve kısa listede aşağıda kalma ihtimali artıyor.`
      : `${rv} This raises perceived risk at decision time and increases the chance of landing lower on the shortlist.`;
  }
  ...
  if (g?.evidence_cv || g?.jd_requirement) {
    ...
    if (langNorm === "tr") {
      return `${ev || "Profilindeki mevcut sinyal"} role tamamen uzak olmadığını gösteriyor. Ama ${req || "rol beklentisi"} tarafında netlik düşük kaldığı için recruiter karar aşamasında tereddüt yaşayabilir.`;
    }
    return `${ev || "Your current profile signal"} suggests you are not fully misaligned. But uncertainty around ${req || "role-specific expectation"} can still make recruiters hesitate at decision time.`;
  }
