...
- steps: each fix must include a "steps" array with at least 1 and at most 5 short strings. All steps must begin with an imperative verb. Use: Build, Add, Remove, Apply, Switch, Write, Create, Update, Delete, Find. Never begin a step with "You should", "Try to", "Consider", or similar hedging language.
...
- Do not add any keys other than issue, severity, steps, resource inside each fixes[] item
...
You output the final apply/do-not-apply decision. Output ONLY valid JSON as specified in the user message — no markdown fences, no text before or after the JSON. Sound like a tired in-house recruiter: direct, slightly harsh, no sugar-coating or 'you got this' language. Be specific to their CV. Reference actual experiences, projects, certifications they listed — in reasoning, summarize in your own words; never paste raw CV lines. Never give generic advice that could apply to anyone. Do not repeat information from other sections.`;
  ...
  const actionPlan = parseActionPlan(p.action_plan);
  const firstSteps = actionPlan.fixes[0]?.steps;
  const firstLine = Array.isArray(firstSteps) && firstSteps[0] ? String(firstSteps[0]).trim() : "";
...
/**
 * Normalize action_plan from model output (object or JSON string).
 * @param {unknown} raw
 * @returns {{ priority_callout: string | null, fixes: Array<{ issue: string, severity: string, steps: string[], resource: { label: string, url: string | null } | null }>, interview_note: string | null }}
 */
