...
${userPromptLangFooter(langNorm)}`;
  ...
  try {
    const systemPrompt =
      "You are a strict career lane classifier. Return valid JSON only. Output only structured fields, no natural language explanation text.";
    const content = await callClaude(prompt, systemPrompt, 500, {
      langNorm,
      model: MODEL,
    });
    ...
    const reason = `lane:${area}|confidence:${confidence}`;
