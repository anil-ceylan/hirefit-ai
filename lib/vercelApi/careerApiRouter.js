...
import { deleteCase as deleteCompanionCase, getCase as getCompanionCase, listCases as listCompanionCases, saveCase as saveCompanionCase, saveOutcome as saveCompanionOutcome } from "../careerCompanion/persistence.js";
import { generatePromotionRaiseGuidance, PROMOTION_RAISE_PROMPT_VERSION } from "../careerCompanion/aiGuidance.js";
import { normalizeCaseInput, validateOutcome } from "../careerCompanion/validation.js";
...
async function handleCareerCompanion(req, res, path, method) {
  const user = await requireUser(req, res); if (!user) return null;
  const match = path.match(/^\/api\/career-companion\/cases\/([^/]+)(?:\/(guidance|outcome))?$/);
  try {
    if (path === "/api/career-companion/cases" && method === "GET") return sendJson(res, 200, { success: true, cases: await listCompanionCases(user.id) });
    if (path === "/api/career-companion/cases" && method === "POST") { const body = await withBody(req); const n = normalizeCaseInput(body); if (!n.ok) return sendJson(res, 400, { success: false, error: "INVALID_INPUT", fields: n.errors }); return sendJson(res, 201, { success: true, case: await saveCompanionCase(user.id, n.value) }); }
    if (!match) return notFound(res);
    const id = decodeURIComponent(match[1]); const action = match[2] || ""; const item = await getCompanionCase(user.id, id); if (!item) return sendJson(res, 404, { success: false, error: "NOT_FOUND" });
    if (!action && method === "DELETE") { await deleteCompanionCase(user.id, id); return sendJson(res, 200, { success: true }); }
    if (!action && method === "GET") return sendJson(res, 200, { success: true, case: item });
    if (action === "guidance" && method === "POST") { const body = await withBody(req); const profile = await loadCareerProfile(user.id); const guidance = await generatePromotionRaiseGuidance({ input: item, profile, lang: body?.lang || "TR" }); const saved = await saveCompanionCase(user.id, { id, guidance, guidance_model: process.env.ANTHROPIC_MODEL || "claude-haiku-4-5-20251001", guidance_prompt_version: PROMOTION_RAISE_PROMPT_VERSION, status: "guided" }); return sendJson(res, 200, { success: true, case: saved }); }
    if (action === "outcome" && method === "POST") { const body = await withBody(req); const check = validateOutcome(body); if (!check.ok) return sendJson(res, 400, { success: false, error: "INVALID_OUTCOME" }); const outcome = await saveCompanionOutcome(user.id, id, { manager_response: String(body.manager_response).trim(), measurable_effect: String(body.measurable_effect || "").trim(), proof_reference: String(body.proof_reference || "").trim(), outcome_date: body.outcome_date || null }); return sendJson(res, 200, { success: true, outcome }); }
    return methodNotAllowed(res);
  } catch (error) { console.error("[career-companion]", error?.message || error); const code = error?.code === "CASE_NOT_FOUND_OR_NOT_OWNED" ? "CASE_NOT_FOUND_OR_NOT_OWNED" : error?.code === "CASE_NOT_DRAFT" ? "CASE_NOT_DRAFT" : error?.code === "MALFORMED_GUIDANCE_OUTPUT" ? "MALFORMED_GUIDANCE_OUTPUT" : error?.code === "STORAGE_UNAVAILABLE" ? "STORAGE_UNAVAILABLE" : "COMPANION_UNAVAILABLE"; return sendJson(res, code === "CASE_NOT_FOUND_OR_NOT_OWNED" ? 404 : code === "CASE_NOT_DRAFT" ? 409 : code === "MALFORMED_GUIDANCE_OUTPUT" ? 502 : 503, { success: false, error: code }); }
}
...
export async function handleCareerApi(req, res, routePath) {
  ...
  try {
    ...
    if (path.startsWith("/api/career-companion/cases")) return handleCareerCompanion(req, res, path, method);
