...
import { getCase as getCompanionCase, listCases as listCompanionCases, saveCase as saveCompanionCase, saveOutcome as saveCompanionOutcome } from "../careerCompanion/persistence.js";
...
async function handleCareerCompanion(req, res, path, method) {
  ...
  } catch (error) { console.error("[career-companion]", error?.message || error); const code = error?.code === "CASE_NOT_FOUND_OR_NOT_OWNED" ? "CASE_NOT_FOUND_OR_NOT_OWNED" : error?.code === "MALFORMED_GUIDANCE_OUTPUT" ? "MALFORMED_GUIDANCE_OUTPUT" : error?.code === "STORAGE_UNAVAILABLE" ? "STORAGE_UNAVAILABLE" : "COMPANION_UNAVAILABLE"; return sendJson(res, code === "CASE_NOT_FOUND_OR_NOT_OWNED" ? 404 : code === "MALFORMED_GUIDANCE_OUTPUT" ? 502 : 503, { success: false, error: code }); }
}
