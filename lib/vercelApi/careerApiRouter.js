...
async function handleCareerCompanion(req, res, path, method) {
  ...
  } catch (error) { console.error("[career-companion]", error?.message || error); return sendJson(res, error?.code === "CASE_NOT_FOUND_OR_NOT_OWNED" ? 404 : error?.code === "MALFORMED_GUIDANCE_OUTPUT" ? 502 : 503, { success: false, error: error?.code || "COMPANION_UNAVAILABLE" }); }
}
