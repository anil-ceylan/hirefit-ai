...
async function groqFetchOnce(model, temperature, messages, responseFormat, max_tokens, langNorm) {
  ...
  const body = { model, temperature, max_tokens: capped, messages: safeMessages };
...
export async function groqChat({
  ...
  try {
    return await groqFetchOnce(model, temperature, messages, responseFormat, max_tokens, langNorm);
  } catch (err) {
    console.warn(`[groqChat] primary model failed (${model}), fallback to ${fallbackModel}:`, err?.message || err);
    return groqFetchOnce(fallbackModel, temperature, messages, responseFormat, max_tokens, langNorm);
  }
}
