...
export async function generatePromotionRaiseGuidance({ input, profile, lang = "TR" }) {
  ...
  const raw = await callClaudeHaiku({ langNorm: tr ? "tr" : "en", max_tokens: 1800, messages: [{ role: "system", content: "You are a careful career conversation coach. Separate facts from assumptions. Do not give legal advice or guarantee an outcome." }, { role: "user", content: prompt }] });
  let parsed;
  try { parsed = JSON.parse(raw); } catch { const match = String(raw || "").match(/\{[\s\S]*\}/); try { parsed = match ? JSON.parse(match[0]) : null; } catch { parsed = null; } }
