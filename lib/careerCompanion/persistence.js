...
function draftOnlyError() { const error = new Error("CASE_NOT_DRAFT"); error.code = "CASE_NOT_DRAFT"; return error; }
export async function deleteCase(userId, caseId) {
  const s = getServiceClient(); if (!s) throw new Error("STORAGE_UNAVAILABLE");
  const { data: existing, error: lookupError } = await s.from("career_companion_cases").select("id, user_id, status").eq("id", caseId).eq("user_id", userId).maybeSingle();
  if (lookupError) throw lookupError;
  if (!existing) throw ownershipError();
  if (existing.status !== "draft") throw draftOnlyError();
  const { error } = await s.from("career_companion_cases").delete().eq("id", caseId).eq("user_id", userId).eq("status", "draft");
