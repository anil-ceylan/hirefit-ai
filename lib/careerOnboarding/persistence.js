...
export async function upsertOnboardingDraft(userId, { step, draft, lang: _lang }) {
  const supabase = getServiceClient();
  if (!supabase || !userId) {
    return { profile: null, storageUnavailable: true };
  }
  const existing = await loadFullCareerProfile(userId);
  const mergedDraft = mergeOnboardingDraft(existing?.onboarding_draft || {}, draft || {}, { step });
  const row = {
    user_id: userId,
...
export async function completeOnboarding(userId, {
  ...
  const cvFields = normalizeCvProfile({
    ...
    ...(cv || {}),
    ...(basic || {}),
    cvStatus: cv?.cvStatus || basic?.cvStatus || existingBasic.cvStatus,
    cvUploaded: cvUploaded ?? cv?.cvUploaded ?? basic?.cvUploaded ?? existingBasic.cvUploaded,
  ...
  const normalizedGoals = normalizeCareerGoals(goals || {});
  ...
  const row = {
    ...
    primary_industry: normalizedGoals?.industries?.[0] || normalizedGoals?.primaryIndustry || "",
    target_countries: normalizedGoals?.targetCountries || basic?.targetCountries || [],
    career_dna: dna,
