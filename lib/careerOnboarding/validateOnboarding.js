import { READINESS_PILLAR_KEYS, normalizeReadinessAnswers } from "./readinessBenchmarks.js";
...
// Answering the disclosure does not require claiming a scored leadership signal.
export function isOnboardingReadinessComplete(readinessAnswers = {}, basic = {}) {
  const { benchmarks } = normalizeReadinessAnswers(readinessAnswers);
  const status = basic?.leadershipExperienceStatus;
  const leadershipAnswered = status === undefined
    ? Boolean(benchmarks.leadership) // Profiles from before the disclosure field existed.
    : ["yes", "no", "unsure"].includes(status);
  return READINESS_PILLAR_KEYS.every((key) =>
    key === "leadership" ? leadershipAnswered : Boolean(benchmarks[key])
  );
}
...
export function validateOnboardingForComplete({
  ...
  if (!isOnboardingReadinessComplete(readinessAnswers, basic)) {
