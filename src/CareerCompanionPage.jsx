...
import { createCareerCompanionCase, generateCareerCompanionGuidance, listCareerCompanionCases, normalizeCareerCompanionCase, saveCareerCompanionOutcome } from "./utils/careerCompanionClient.js";
...
export default function CareerCompanionPage() {
  ...
  return <main className="hf-companion-page"><div className="hf-companion-shell">
    ...
    {error ? <div className="hf-companion-alert" role="alert"><span>{error}</span>{selected?.id && !selected.guidance ? <button type="button" className="hf-btn-secondary" onClick={retryGuidance} disabled={busy}>{tr ? "Rehberliği tekrar dene" : "Retry guidance"}</button> : <button type="button" className="hf-btn-secondary" onClick={load} disabled={busy}>{tr ? "Tekrar dene" : "Retry"}</button>}</div> : null}
