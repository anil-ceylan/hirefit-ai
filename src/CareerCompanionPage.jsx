...
import { createCareerCompanionCase, deleteCareerCompanionCase, generateCareerCompanionGuidance, listCareerCompanionCases, normalizeCareerCompanionCase, saveCareerCompanionOutcome } from "./utils/careerCompanionClient.js";
...
export default function CareerCompanionPage() {
  ...
  const replaceCase = (item) => { const normalized = normalizeCareerCompanionCase(item); setSelected(normalized); setCases((current) => [normalized, ...current.filter((entry) => entry.id !== normalized.id)]); };
  const retryGuidance = async () => { if (busy || !selected?.id) return; setBusy(true); setError(""); try { const guided = await generateCareerCompanionGuidance(getApiAuthHeaders, selected.id, lang); replaceCase(guided.case); } catch (e) { setError(e.message); } finally { setBusy(false); } };
  const create = async (event) => { event.preventDefault(); if (busy) return; if (selected?.id && !selected.guidance) { await retryGuidance(); return; } setError(""); if (!validateCase()) return; setBusy(true); try { const data = await createCareerCompanionCase(getApiAuthHeaders, form); replaceCase(data.case); try { const guided = await generateCareerCompanionGuidance(getApiAuthHeaders, data.case.id, lang); replaceCase(guided.case); } catch (e) { setError(e.message); } } catch (e) { setError(e.message); } finally { setBusy(false); } };
  const reopen = (item) => { const normalized = normalizeCareerCompanionCase(item); setSelected(normalized); setForm({ title: normalized.title || "", what_happened: normalized.what_happened || "", desired_outcome: normalized.desired_outcome || "", role_context: normalized.role_context || "", achievements: normalized.achievements || "", urgency: normalized.urgency || "medium", manager_context: normalized.manager_context || "" }); setFormErrors({}); setError(""); setMessage(""); };
  const removeDraft = async (event, item) => { event.stopPropagation(); if (item.status !== "draft" || busy || !window.confirm("Bu taslağı kaldırmak istediğine emin misin?")) return; setBusy(true); setError(""); setMessage(""); try { await deleteCareerCompanionCase(getApiAuthHeaders, item.id); setCases((current) => current.filter((entry) => entry.id !== item.id)); setSelected((current) => current?.id === item.id ? null : current); setMessage("Taslak kaldırıldı."); } catch (e) { setError(e.message); } finally { setBusy(false); } };
  ...
  return <main className="hf-companion-page"><div className="hf-companion-shell">
    ...
    {message ? <div className="hf-companion-alert hf-companion-success" role="status">{message}</div> : null}{error ? <div className="hf-companion-alert" role="alert"><span>{error}</span>{selected?.id && !selected.guidance ? <button type="button" className="hf-btn-secondary" onClick={retryGuidance} disabled={busy}>{tr ? "Rehberliği tekrar dene" : "Retry guidance"}</button> : <button type="button" className="hf-btn-secondary" onClick={load} disabled={busy}>{tr ? "Tekrar dene" : "Retry"}</button>}</div> : null}
    <section className="hf-card hf-companion-card"><div className="hf-companion-section-heading"><div><p className="hf-companion-eyebrow">{tr ? "İlk adım" : "First step"}</p><h2>{tr ? "Terfi / Zam" : "Promotion / Raise"}</h2></div><span className="hf-companion-status">{tr ? "6 kısa alan" : "6 short fields"}</span></div>
      <form onSubmit={create} noValidate className="hf-companion-form">
        ...
        <div className="hf-companion-form-actions"><button type="submit" disabled={busy} className="hf-btn-primary">{busy ? (tr ? "Hazırlanıyor..." : "Preparing...") : (selected?.id && !selected.guidance ? (tr ? "Rehberliği tekrar dene" : "Retry guidance") : (tr ? "Kaydet ve rehberlik oluştur" : "Save and create guidance"))}</button></div>
      </form>
    </section>
    ...
    <section className="hf-companion-saved"><div className="hf-companion-section-heading"><h2>{tr ? "Kayıtlı vakalar" : "Saved cases"}</h2></div>{cases.length ? <div className="hf-companion-case-list">{cases.map((item) => <div className="hf-companion-case" key={item.id}><button type="button" className="hf-companion-case-main" onClick={() => reopen(item)}><span>{item.title || (tr ? "Terfi / Zam vakası" : "Promotion / Raise case")}</span><span>{displayStatus(item.status)}</span></button>{item.status === "draft" ? <button type="button" className="hf-btn-secondary hf-companion-delete" onClick={(event) => removeDraft(event, item)} disabled={busy}>{tr ? "Taslağı kaldır" : "Remove draft"}</button> : null}</div>)}</div> : <p className="hf-companion-empty">{tr ? "Henüz kayıt yok. İlk görüşmeni yukarıdaki formdan planla." : "No saved cases yet. Plan your first conversation above."}</p>}</section>
  </div></main>;
}
